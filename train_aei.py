...
try:
    ...
    G.load_state_dict(torch.load('./saved_models/G_train.pth', map_location=torch.device('cpu')), strict=False)
    D.load_state_dict(torch.load('./saved_models/D_train.pth', map_location=torch.device('cpu')), strict=False)
...
def get_grid_image(X):
    ...
    X = (torchvision.utils.make_grid(X.detach().cpu(), nrow=X.shape[0]) + 1) / 2 * 255
    return X
...
for epoch in range(0, max_epoch):
    for iteration, data in enumerate(dataloader):
        ...
        if iteration % show_step == 0:
            ...
            cv2.imwrite('./gen_images/latest{}.jpg'.format(epoch), image.transpose([1,2,0]))
            writer.add_scalar('lossD', lossD.item(), global_iter)
            writer.add_scalar('lossG', lossG.item(), global_iter)
            writer.add_scalar('L_adv', L_adv.item()*l_adv, global_iter)
            writer.add_scalar('L_id', L_id.item()*l_id, global_iter)
            writer.add_scalar('L_attr', L_attr.item()*l_att, global_iter)
            writer.add_scalar('L_rec', L_rec.item()*l_rec, global_iter)
        ...
        if iteration % 100 == 0:
            torch.save(G.state_dict(), './saved_models/G_train.pth')
            torch.save(D.state_dict(), './saved_models/D_train.pth')
